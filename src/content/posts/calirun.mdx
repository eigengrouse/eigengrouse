...
title: 'CaliRun'
pubDate: 2026-02-09
description: 'An OutRun-inspired ZX Spectrum racer mixing BASIC with machine code.'
author: 'eigengrouse'
image: '/calirun.png'
tags: ["zx-spectrum", "games"]
tapFile: '/games/calirun.tap'
---

import JSSpeccy from '../../components/JSSpeccy.astro';

An OutRun-inspired racing game written in Sinclair BASIC, with a machine code scroll routine
assembled from Z80 into BASIC `DATA` statements via a build pipeline. The `.tap` file is for use
with a ZX Spectrum emulator, tested with Fuse.
...
There is some interesting stuff going on here, including a build pipeline in `make.bat` that
assembles Z80 into BASIC `DATA` statements for two machine code routines and the UDGs.
[Pasmo](https://pasmo.speccy.org/pasmodoc.html) is used to assemble the `.asm` files into hex, which
is then converted into `DATA` statements using some custom PowerShell scripts. The generated code,
found in `build.zxb`, is all in Sinclair BASIC. That is then built into a `.tap` file using
[zmakebas](https://github.com/z00m128/zmakebas).

The core mechanic of this game is a short machine code routine that scrolls the attribute data from
the top row down to one row above the bottom, leaving room for game data to be displayed on the
bottom row. The routine makes use of the Z80 [`lddr`](https://jnz.dk/z80/lddr.html) instruction,
which can move memory very quickly, and transferring only the attribute data (colour information) is
far quicker than transferring bitmap data (pixel information), by a factor of eight. The routine is
assembled into `DATA` statements from the `.asm` file and appended to the BASIC file, as shown
below.
```asm
...
This means the graphics are either blocky, or made up of pre-filled characters that are turned on
and off by updating the colour information. Individual updates can be done with the BASIC `PRINT`
command, which is reasonably quick but still has the overhead of updating bitmap data as well. One
extra optimisation is to update the attribute data directly using `POKE`. That lets us update a
memory address directly, which we can express as `POKE a, v` where `a=22528+ROW*32+COLUMN` and
`v=INK+(PAPER×8)+(BRIGHT×64)+(FLASH×128)`.

#### Sinclair BASIC implementation

I used [zmakebas](https://github.com/z00m128/zmakebas) again for the BASIC, which lets you use
labels instead of line numbers, but for completeness I opened the generated TAP file in
[BasinC](https://github.com/ref-xx/basinc), an IDE that reads and reformats your code into idiomatic
Sinclair BASIC. I have pasted that below. I did not do any custom font loading this time, so the
following listing can be typed in to create the whole game, although the comments have been
removed by the process. One nice thing about this is that it makes me think about how BASIC games
could be, and were, distributed in magazines and cover tapes back in the day.
